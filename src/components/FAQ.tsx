...
const FAQ = () => {
  ...
  return (
    <section className="py-16 bg-gradient-to-b from-gray-50 to-yellow-50">
          ...
          {faqItems.map((item, index) => (
            <div key={index} className="bg-gray-800 rounded-lg overflow-hidden">
              <button
                ...
                className="w-full px-6 py-4 text-left flex items-center justify-between text-white font-bold hover:bg-gray-700 transition-colors"
              >
                <span className="flex items-center">
                  <svg 
                    className={`w-5 h-5 mr-3 transition-transform ${openIndex === index ? 'rotate-180' : ''}`}
                    fill="none" 
                    stroke="currentColor" 
                    viewBox="0 0 24 24"
                  >
